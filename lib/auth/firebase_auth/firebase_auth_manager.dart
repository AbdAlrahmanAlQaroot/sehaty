...
class FirebaseAuthManager extends AuthManager
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'مر وقت طويل منذ آخر تسجيل دخول. قم بتسجيل الدخول مرة أخرى قبل حذف حسابك.')),
        );
      }
    }
  }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'مر وقت طويل منذ آخر تسجيل دخول. قم بتسجيل الدخول مرة أخرى قبل إعادة تعيين بريدك الإلكتروني.')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('حدث خطأ في المصادقة')),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
    ...
    phoneAuthManager.addListener(() {
      ...
      if (phoneAuthManager.triggerOnCodeSent) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('حدث خطأ في المصادقة'),
        ));
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : SehatyFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('حدث خطأ في المصادقة')),
